...
public class InferenceController : MonoBehaviour
{
    ...
    [SerializeField] ScreenToWorldConversion screenToWorldConversion;
    ...
    private void Update()
    {
        ...
        GetInformation(bboxInfoArray);
    }

    #endregion

    private void GetInformation(BBox2DInfo[] bboxInfoArray){
        for (int i = 0; i < bboxInfoArray.Length; i++) {            
            ...
            Vector3 result ;
            Ray ray = camera.ScreenPointToRay(new Vector3(center.x, center.y, 0)); 
            RaycastHit hit;
            if (Physics.Raycast(ray, out hit)) {                
                result = new Vector3(center.x, center.y, hit.distance);                             
                screenToWorldConversion.ScreenPointToWorldPoint(result, bboxInfo.label);
            }
            // Debug.Log(bboxInfo.label);
            // Debug.Log(center);
        }        
    }
